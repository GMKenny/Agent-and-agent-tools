turtles-own [ wealth happy neighbor total_neighbors give_amount]
...
to setup
  ...
  create-turtles 500 [
    ...
    set color green
    set size 2
    ;; Happy determines if the turtle is willing to give away money
    set happy one-of [ 0 1 ]
    ;; Give_amount determines how much one turtle is willing to give away
    set give_amount one-of [ 1 3 ]
...
to see
  ;; Get closet turtle
  set neighbor min-one-of other turtles [distance myself]
  ;; Get total neighbors in radius 5
  set total_neighbors count turtles in-radius 5
end

to act
  ;; If there are more than 5 closes neighbors mood is either 0 or 1 (true or false) else 1 (true)
  set happy ifelse-value (total_neighbors > 5 ) [(one-of [ 0 1 ])] [1]
  ;; If current give amount is equal to 1 net give amount is either 1 or 2  (Greed)
  ;; Else give amount is a number between 1 to 4
  set give_amount ifelse-value (give_amount = 1) [(one-of [ 1 2])] [(one-of [ 1 4])]
  ;; If the give amount is higher than the wealth give amount is equal to wealth
  set give_amount ifelse-value (give_amount > wealth) [wealth] [give_amount]
end

to update
  ;; If happy is false color red else green
  set color ifelse-value (happy = 0) [red] [green]
  ;; If wealth is above - and happy is 1 (true) tansact
  if wealth > 0 and happy = 1 [ transact ]
  ;; Set postion based on wealth
  if wealth <= max-pxcor [ set xcor wealth ]
end

to transact
  ;; Current wealth is wealth - give amount
  set wealth wealth - give_amount
  ;; Ask neigbor increase wealth with give amount
  ask neighbor [ set wealth wealth + give_amount ]
end
